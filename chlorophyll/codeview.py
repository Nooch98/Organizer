...
toml_files = [archivo.name for archivo in color_schemes_dir.glob("*.toml")]
...
class CodeView(Text):
    _w: str
    _builtin_color_schemes = set(toml_files)
    ...
    def _set_color_scheme(self, color_scheme: dict[str, dict[str, str | int]] | str | None) -> None:
        if isinstance(color_scheme, str):
            theme_path = color_schemes_dir / f"{color_scheme}.toml"
            if theme_path.exists():
                color_scheme = toml.load(theme_path)
            else:
